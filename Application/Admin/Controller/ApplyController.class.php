...
class ApplyController extends BackendController
{
    ...
    {
        if (false === $apply = F('apply_info_list')) {
            $apply = D('Apply')->apply_info_cache();
        }
    ...
    protected function _menu($sql)
    {
        ...
        if (!preg_match_all($reg, $arr[0], $reg)) {
            return false;
        }
        $fields = $reg[1];
    ...
    protected function _menu_add($menus, $fid = 0, &$spids)
    {
        foreach ($menus as $key => $val) {
            ...
            $spid = $spids[$pid].$mid.'|';
            if (false === M('Menu')->where(array('id'=>$mid))->save(array('pid'=>$pid,'spid'=>$spid))) {
                return false;
            }
            M('AdminAuth')->add(array('role_id' => 1,'menu_id' => $mid));
            if ($val['son']) {
                if (false === $this->_menu_add($val['son'], $mid, $spids)) {
                    return false;
                }
            }
        }
