...
class SmsQueueController extends BackendController
{
    ...
    public function index()
    {
        ...
        if ($key && $key_type>0) {
            switch ($key_type) {
                case 1:
                    $map['s_body']=array('like','%'.$key.'%');
                    break;
                case 2:
                    $map['s_mobile']=array('eq',$key);
                    break;
            }
        } else {
    ...
    public function smsqueue_add()
    {
        if (IS_POST) {
            ...
            foreach ($mobile_arr as $key => $value) {
                preg_match("/^(13|14|15|17|18|16|19)\d{9}$/", $value) && $m_arr[] = $value;
            }
            $num = 0;
            if (!empty($m_arr)) {
    ...
    public function smsqueue_edit()
    {
        if (IS_POST) {
            ...
            $s_body=I('post.s_body', '', 'trim')?I('post.s_body', '', 'trim'):$this->error('请填写短信内容');
            mb_strlen($s_body, 'utf-8')>70 && $this->error('短信内容超过70个字，请重新输入！');
            if (preg_match("/^(13|14|15|17|18|16|19)\d{9}$/", $setsqlarr['s_sms'])) {
                $_POST['s_type']=1;
                $_POST['s_body']=$s_body;
    ...
    public function smsqueue_batchadd()
    {
        if (IS_POST) {
            ...
            foreach ($result as $key => $value) {
                if (preg_match("/^(13|14|15|17|18|16|19)\d{9}$/", $value['mobile'])) {
