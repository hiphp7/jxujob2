...
class Builder extends ZBuilder
{
    ...
    public function _initialize()
    {
        ...
        $this->_vars['post_url']    = '';
        $this->_vars['_token_name'] = C('zbuilder.form_token_name');
    ...
    {
        $size = ($size != '' ? $size : C('upload_file_size')) * 1024;
        $ext  = $ext != '' ? $ext : C('upload_file_ext');
    ...
    public function addImageurl($name = '', $title = '', $tips = '', $default = '', $size = '', $ext = '', $extra_class = '', $thumb = '', $watermark = '')
    {
        $size = ($size != '' ? $size : C('upload_image_size')) * 1024;
        $ext  = $ext != '' ? $ext : C('upload_image_ext');
    ...
    public function __call($methodName, $argument)
    {
        ...
        if ($type != '') {
            $class_name = 'Common\\Builder\\form\\extend\\' . $type . '\\Builder';
    ...
    private function parseUrl($urls = [], $type = '')
    {
        foreach ($urls as $key => $item) {
            if (strpos($item, '/') === 0) {
                ...
                $urls[$key] = '/extend/form/' . $type . '/' . $item;
            }
        }
        return $urls;
    }
