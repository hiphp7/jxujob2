...
class Builder extends ZBuilder
{
    ...
    public function _initialize()
    {
        ...
        $this->_controller = parse_name(CONTROLLER_NAME);
        $this->_action     = ACTION_NAME;
        $this->_table_name = CONTROLLER_NAME;
        $this->_template   = '../../../Common/Builder/table/layout';
            ...
            if (!empty($options) && is_array($options)) {
                foreach ($columns as $key => $column) {
                    if (is_numeric($key)) {
                        ...
                        S('filter_options_' . $column, $options);
                        $this->_filter_options[$column] = 'filter_options_' . $column;
                    } else {
                        S('filter_options_' . $key, $options);
     ...
     * @return array
     */
    protected function buildFilterMap($map = '')
    {
        if (is_array($map)) {
            return $map;
        }
        ...
        if ($_pos !== false) {
            $_filter_content = I('_filter_content');
     ...
     * @return string
     */
    protected function getDefaultUrl($type = '', $params = [])
    {
     ...
     * @return bool
     */
    protected function checkButtonAuth($btn_attribute = [])
    {
     ...
     * @return bool|string
     */
    protected function createTableToken($table = '', $prefix = 1)
    {
        $data = [
            'table'      => $table, // 表名或模型名
            'prefix'     => $prefix,
    ...
    public function addRightButton($type = '', $attribute = [], $pop = false, $extra = [])
    {
        ...
        // 按钮样式
        $btn_style = array_merge(C('zbuilder.right_button'), $extra);
    ...
    public function setSearch($fields = [], $placeholder = '', $url = '', $search_button = null)
    {
        if (!empty($fields)) {
            ...
            $this->_vars['search_button'] = $search_button !== null ? $search_button : C('zbuilder.search_button');
        }
        return $this;
    }
     ...
     * @author caiweiming <314013107@qq.com>
     */
    protected function loadFile($type = '', $files_name = '', $module = '')
    {
        if ($files_name != '') {
            ...
            foreach ($files_name as $item) {
                if (strpos($item, '/') === 0) {
                    $this->_vars[$type . '_list'][] = $item . '.' . $type;
                } else {
                    $this->_vars[$type . '_list'][] = '/Public/' . $module . '/' . $type . '/' . $item . '.' . $type;
                }
            }
        }
    }
     ...
     * @return array
     */
    protected function toArray($row_list)
    {
        if ($row_list instanceof \think\paginator) {
     ...
     * @return mixed
     */
    protected function getData($index = '', $field = '')
    {
     ...
     * @return array
     */
    protected function parseTrClass($row)
    {
        $_tr_class = [];
     ...
     * @return bool
     */
    protected function parseCondition($row, $condition = [])
    {
        $match = true;
        switch ($condition[1]) {
            case 'eq':
     ...
     * @return bool|string
     */
    protected function createFilterToken($table = '', $field = '')
    {
    ...
    protected function compileTable()
    {
        ...
        if ($this->_vars['_select_list']) {
            foreach ($this->_vars['_select_list'] as $name => &$select) {
                ...
                // 要搜索的字段
                $select_field = I('_select_field', '');
                $select_field = $select_field != '' ? explode('|', $select_field) : [];

                // 对应的值
                $select_value = I('_select_value', '');
        ...
        if ($this->_search) {
            ...
            $this->_vars['search'] = [
                ...
                'url'         => $this->_search['url'] == '' ? U($this->_controller . '/' . $this->_action) : $this->_search['url'],
            ];
        }
