 ...
 * );
 */
function build_subject_url($data = array()) {
    $config = F('Mobile/Conf/config', '', APP_PATH);
    $url = C('qscms_site_domain') . C('qscms_site_dir');
    if (empty($data)) {
        if ($config['URL_MODEL'] == 0) {
        ...
        if ($config['URL_MODEL'] == 0) {
			...
			if(!C('subsite_info.s_id')){
				...
				if(C('subsite_info.s_m_domain')){
					$url = C('subsite_info.mobile_type').C('subsite_info.s_m_domain'). '/'. 'Subject/' . $data['c'] . '/' . $data['a'] . ($data['params'] ? ('/' . $data['params']) : '');
				}else{
				    $url = C('subsite_info.pc_type').C('subsite_info.s_domain') . '/'. 'Subject/' . $data['c'] . '/' . $data['a'] . ($data['params'] ? ('/' . $data['params']) : '');
				}
...
//调用接口并转化php数组
function https_request_array($url,$data = null){
    if(function_exists('curl_init')){
        $curl = curl_init();
        curl_setopt($curl, CURLOPT_URL, $url);
        curl_setopt($curl, CURLOPT_SSL_VERIFYPEER, FALSE);
...
function getExcel($property,$headArr,$data,$fileName){
