...
use Think\Model;
class GworkerCategoryModel extends Model
{
    public function category_cache() {
        $category = array();
        $categoryData = $this->field('id,alias,name')->order('ordid desc,id asc')->select();
        foreach ($categoryData as $key=>$val){
        	$category[$val['alias']][$val['id']] = $val['name'];
        }
        F('gworker_category', $category);
        return $category;
    }
    /**
    ...
    {
        if(false === $category = F('gworker_category')){
