...
class CallbackController extends FrontendController
{
    ...
    public function index()
    {
        ...
        if ('unbind' == $type) {
            ...
            if (false === M('MembersBind')->where(array('uid'=>C('visitor.uid'), 'type'=>$mod))->delete()) {
                $this->error('解除绑定失败，请重新操作！');
            }
    ...
    public function oauth()
    {
        ...
        if (false === $url) {
            $this->error($oauth->getError(), __ROOT__);
        }
        redirect($url);
    }
    /**
        ...
        if (false !== F('/weixin/'.($event_key%10).'/'.$event_key)) {
            ...
            if ($reg['state']) {
                ...
                if (false === $data = $passport->register($data)) {
                    return array('state'=>0,'tip'=>$passport->get_error());
                }
