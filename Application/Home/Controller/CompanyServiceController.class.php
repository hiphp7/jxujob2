...
class CompanyServiceController extends CompanyController
{
    ...
    public function _initialize()
    {
        parent::_initialize();
        //访问者控制
        if (!$this->visitor->is_login && IS_AJAX) {
            $this->ajaxReturn(0, L('login_please'), '', 1);
        }
        if (C('visitor.utype') !=1 && IS_AJAX) {
            $this->ajaxReturn(0, '请登录企业帐号！');
        }
    ...
    public function jobs_refresh()
    {
        ...
        if (IS_POST) {
            ...
            if (!$this->my_setmeal) {
                $this->ajaxReturn(0, '您还没有开通服务，请<a target="_blank" href="'.U('CompanyService/index').'">开通</a>！');
            }
            if ($this->my_setmeal['expire']==1 && $this->my_setmeal['setmeal_id']>1) {
                $this->ajaxReturn(0, '您的服务已经到期，请<a target="_blank" href="'.U('CompanyService/index').'">重新开通</a>！');
            }
