...
class PersonalController extends FrontendController
{
    ...
    protected function _is_resume($pid)
    {
        ...
        if (!$reg = M('Resume')->field()->where(array('id'=>$pid,'uid'=>C('visitor.uid')))->find()) {
            return false;
        }
    ...
    */
    public function index()
    {
        ...
        if ($resume_info['intention_jobs_id']) {
            ...
            if (false === $result = F('jobs_cate_list')) {
    ...
    public function ajax_get_interest_jobs($type = '')
    {
        ...
        if (!in_array($type, array('recommend_jobs','nearby_jobs','new_jobs'))) {
            $this->ajaxReturn(0, '数据类型错误！');
        }
        $where = array(
            '显示数目' => '12',
            '分页显示' => 1
    ...
    */
    ...
    {
        if (false === $resume = $this->_is_resume()) {
    ...
    public function refresh_resume()
    {
        if (IS_AJAX) {
            ...
            if (C('qscms_per_refresh_resume_time')!=0&&($count>=C('qscms_per_refresh_resume_time'))) {
                ...
                if ($data) {
                    if (true !== $reg = D('Members')->update_user_info($data, C('visitor'), array('id'=>$pid))) {
    ...
    public function save_shield_company()
    {
        ...
        if (10 <= $count = M('PersonalShieldCompany')->where($data)->count()) {
            $this->ajaxReturn(0, '您最多屏蔽 10 个企业关键词！');
        }
        $data['comkeyword'] = $keyword;
        $shield_mod = D('PersonalShieldCompany');
        if (false === $shield_mod->create($data)) {
            $this->ajaxReturn(0, $shield_mod->getError());
        }
        if (false === $data['id'] = $shield_mod->add()) {
            $this->ajaxReturn(0, '企业关健字添加失败，请重新添加！');
        }
        //写入会员日志
    ...
    */
    ...
    public function copy_resume()
    {
        ...
        if (!$reg['state']) {
            $this->error($reg['error'], 'resume_list');
        }
    ...
    */
    public function resume_edit()
    {
        ...
        if (IS_POST) {
            ...
            if (1 == $auto_apply=I('post.auto_apply', 0, 'intval')) {// 3天内 自动投递
                if (true !== $reg = D('ResumeEntrust')->set_resume_entrust($resume['id'], $uid)) {
                    $this->error($reg);
                }
    ...
    public function ajax_save_basic_info()
    {
        ...
        if (C('qscms_audit_edit_resume')!="-1") {
            D('ResumeEntrust')->set_resume_entrust($resume['id'], $uid);//添加简历自动投递功能
        }
    ...
    */
    public function ajax_save_basic()
    {
        ...
        if (C('qscms_audit_edit_resume')!="-1") {
            D('ResumeEntrust')->set_resume_entrust($pid, $uid);//添加简历自动投递功能
        }
        $rst=D('Resume')->save_resume($setsqlarr, $pid, C('visitor'));
        if ($rst['state']) {
            $this->ajaxReturn(1, '求职意向修改成功！', $rst['attach']);
        }
        $this->ajaxReturn(0, $rst['error']);
    }
    /**
    ...
    protected function _del_data($type)
    {
        ...
        if (IS_POST) {
            ...
            if (M($type)->where(array('id'=>$id,'uid'=>$uid,'pid'=>$pid))->delete()) {
                switch ($type) {
                    case '	':
                        write_members_log($user, 2013, $pid);
                        break;
                    case 'ResumeWork':
                        write_members_log($user, 2014, $pid);
                        break;
                    case 'ResumeTraining':
                        write_members_log($user, 2015, $pid);
                        break;
                    case 'ResumeLanguage':
                        write_members_log($user, 2016, $pid);
                        break;
                    case 'ResumeCredent':
                        write_members_log($user, 2017, $pid);
                        break;
                }
                $resume_mod = D('Resume');
    ...
    */
    ...
    public function save_education()
    {
        ...
        if ($setsqlarr['todate'] == 1) {
            ...
            if ($setsqlarr['startyear'] > intval(date('Y'))) {
                $this->ajaxReturn(0, '就读开始时间不允许大于当前时间！');
            }
            if ($setsqlarr['startyear'] == intval(date('Y')) && $setsqlarr['startmonth'] >= intval(date('m'))) {
                $this->ajaxReturn(0, '就读开始时间需小于当前时间！');
            }
            if ($setsqlarr['endyear'] > intval(date('Y'))) {
                $this->ajaxReturn(0, '就读结束时间不允许大于当前时间！');
            }
    ...
    public function save_work()
    {
        ...
        if ($setsqlarr['todate'] == 1) {
            if (!$setsqlarr['startyear'] || !$setsqlarr['startmonth']) {
                $this->ajaxReturn(0, '请选择工作时间！');
            }
            if ($setsqlarr['startyear'] > intval(date('Y'))) {
                $this->ajaxReturn(0, '工作开始时间不允许大于当前时间！');
            }
            if ($setsqlarr['startyear'] == intval(date('Y')) && $setsqlarr['startmonth'] >= intval(date('m'))) {
                $this->ajaxReturn(0, '工作开始时间需小于当前时间！');
            }
        } else {
            if (!$setsqlarr['startyear'] || !$setsqlarr['startmonth'] || !$setsqlarr['endyear'] || !$setsqlarr['endmonth']) {
                $this->ajaxReturn(0, '请选择工作时间！');
            ...
            if ($setsqlarr['endyear'] > intval(date('Y'))) {
                $this->ajaxReturn(0, '工作结束时间不允许大于当前时间！');
            }
            if ($setsqlarr['endyear'] == intval(date('Y')) && $setsqlarr['endmonth'] > intval(date('m'))) {
                $this->ajaxReturn(0, '工作结束时间不允许大于当前时间！');
            }
            ...
            if ($setsqlarr['startyear'] > $setsqlarr['endyear']) {
                $this->ajaxReturn(0, '工作开始时间不允许大于结束时间！');
            }
            if ($setsqlarr['startyear'] == $setsqlarr['endyear'] && $setsqlarr['startmonth'] >= $setsqlarr['endmonth']) {
                $this->ajaxReturn(0, '工作开始时间需小于结束时间！');
            }
        }
        if (false === $resume = $this->_is_resume()) {
            $this->ajaxReturn(0, '请先填写简历基本信息！');
    ...
    public function save_training()
    {
        ...
        if ($setsqlarr['todate'] == 1) {
            if (!$setsqlarr['startyear'] || !$setsqlarr['startmonth']) {
                $this->ajaxReturn(0, '请选择培训时间！');
            }
            if ($setsqlarr['startyear'] > intval(date('Y'))) {
                $this->ajaxReturn(0, '培训开始时间不允许大于毕业时间！');
            }
            if ($setsqlarr['startyear'] == intval(date('Y')) && $setsqlarr['startmonth'] >= intval(date('m'))) {
                $this->ajaxReturn(0, '培训开始时间需小于毕业时间！');
            }
        } else {
            if (!$setsqlarr['startyear'] || !$setsqlarr['startmonth'] || !$setsqlarr['endyear'] || !$setsqlarr['endmonth']) {
                $this->ajaxReturn(0, '请选择培训时间！');
            ...
            if ($setsqlarr['startyear'] > intval(date('Y'))) {
                $this->ajaxReturn(0, '培训开始时间不允许大于当前时间！');
            }
            if ($setsqlarr['startyear'] == intval(date('Y')) && $setsqlarr['startmonth'] >= intval(date('m'))) {
                $this->ajaxReturn(0, '培训开始时间需小于当前时间！');
            }
            if ($setsqlarr['endyear'] > intval(date('Y'))) {
                $this->ajaxReturn(0, '培训结束时间不允许大于当前时间！');
            }
            if ($setsqlarr['endyear'] == intval(date('Y')) && $setsqlarr['endmonth'] > intval(date('m'))) {
                $this->ajaxReturn(0, '培训结束时间不允许大于当前时间！');
            }
    ...
    public function save_credent()
    {
        ...
        if (!$setsqlarr['year'] || !$setsqlarr['month']) {
            $this->ajaxReturn(0, '请选择获得证书时间！');
        }
        if ($setsqlarr['year'] > intval(date('Y'))) {
            $this->ajaxReturn(0, '获得证书时间不能大于当前时间！');
        }
    ...
    */
    public function ajax_resume_img_del()
    {
        if (IS_POST) {
            ...
            if (false === $img_mod->where(array('id'=>$img_id,'uid'=>$uid))->delete()) {
                $this->ajaxReturn(0, '删除失败！');
            }
            //写入会员日志
    ...
    */
    public function ajax_word_del()
    {
        ...
        if ($warning) {
            ...
            if ($resume['word_resume']) {
                ...
                $resume_mod = D('Resume');
                if (false === $resume_mod->where(array('id'=>$resume['id']))->setfield('word_resume', '')) {
    ...
    public function outward_save()
    {
        ...
        if (false === $resume = $this->_is_resume($setsqlarr['resume_id'])) {
            $this->ajaxReturn(0, '简历不存在或已经删除!');
        }
    ...
    public function del_outward()
    {
        if (IS_AJAX) {
            ...
            if (!fieldRegex($yid, 'in')) {
                $this->error('请正确选简历外发记录!', U('outward'));
            }
            $num = M('ResumeOutward')->where(array('id'=>array('in',$yid)))->delete();
            if (!$num) {
                $this->error('删除失败！', U('outward'));
            }
            //写入会员日志
    ...
    */
    ...
    public function user_info()
    {
        ...
        if (IS_POST) {
            ...
            if (true !== $reg = D('Members')->update_user_info($setsqlarr, C('visitor'))) {
                $this->ajaxReturn(0, $reg);
            }
