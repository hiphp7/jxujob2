<?php
namespace Locoyspider\Model;
use Think\Model;
class LocoyspiderModel extends Model{
	//获取采集配置数据二维数组
	...
	public function addJobs($companyinfo){
		...
		$post['setmeal_id']=$locoyspider['reg_service'];
		$setmeal=D('Setmeal')->get_setmeal_one($post['setmeal_id']);
		$post['setmeal_name']=$setmeal['setmeal_name'];
		...
		$mspost['setmeal_id']=$post['setmeal_id'];
		$mspost['setmeal_name']=$post['setmeal_name'];
		$mspost['starttime']=time();
		M("members_setmeal")->where(array('uid'=>$post['uid']))->save($mspost);
		...
		$searchtab['setmeal_id']=$post['setmeal_id'];
	...
	public function addCompany($companyname){
		...
		$setsqlarr['landline_tel']=trim($_POST['landline_tel']);
