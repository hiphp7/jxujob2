...
class CompanyController extends MobileController
{
    ...
    public function set_allowance_job_save()
    {
        ...
        if ($setsqlarr['per_amount']<$config['amount_range'][0]) {
            $this->ajaxReturn(0, '红包金额不能小于'.$config['amount_range'][0].'元');
        }
        if ($setsqlarr['per_amount']>$config['amount_range'][1]) {
            $this->ajaxReturn(0, '红包金额不能大于'.$config['amount_range'][1].'元');
        }
    ...
    public function allowance_wxpay($info_id)
    {
        ...
        $paysetarr['ordsubject'] = $model->type_alias_cn[$info['type_alias']];
        $paysetarr['ordbody'] = $model->type_alias_cn[$info['type_alias']];
        $paysetarr['openid'] = D('MembersBind')->where(array('uid'=>$info['uid'],'type'=>'weixin'))->getField('keyid');
        $r = D('Payment')->pay($paysetarr);
