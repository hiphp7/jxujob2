...
class MembersController extends MobileController
{
    ...
    public function login()
    {
        if (IS_POST) {
            ...
            if (C('qscms_mobile_captcha_open')==1 && (C('qscms_wap_captcha_config.user_login')==0 || (session('?error_login_count') && session('error_login_count')>=C('qscms_wap_captcha_config.user_login')))) {
                if (true !== $reg = \Common\qscmslib\captcha::verify('mobile')) {
    ...
    {
        if (C('qscms_mobile_captcha_open') && C('qscms_wap_captcha_config.varify_mobile') && true !== $reg = \Common\qscmslib\captcha::verify('mobile')) {
            $this->ajaxReturn(0, $reg);
        }
        ...
        if (IS_POST && IS_AJAX) {
            ...
            if ('bind' == I('post.org', '', 'trim') && cookie('members_bind_info')) {
                ...
                if ($user_bind_info['type'] == 'weixin') {
                    ...
                    if ($user_bind_info['keyavatar_big']) {
                        M('Members')->where(array('uid'=>$data['uid']))->setfield('avatars', $user_bind_info['keyavatar_big']);//临时头像转换
                    }
                } else {
    ...
    public function activate()
    {
        ...
        if ($end_time<time()) {
            $this->_404('注册失败,链接过期!', U('members/register'));
        }
        $key_str=substr(md5($data['e'].$data['t']), 8, 16);
        if ($key_str!=$data['k']) {
            $this->_404('注册失败,key错误', U('members/register'));
        }
        $members_mod = D('Members');
        ...
        if ($user['status'] == 0) {
            ...
            if (false === $uid = $passport->edit($user['uid'], $d)) {
                $this->_404('帐号激活失败，请重新操作！', U('members/register'));
            }
            $user['reg_type'] = 2;
    ...
    public function user_setpass()
    {
        if (IS_POST) {
            ...
            if ($key) {
                ...
                $end_time=$data['t']+24*3600;
                if ($end_time<time()) {
                    $this->_404('找回密码失败,链接过期!', 'user_getpass');
                }
                $key_str=substr(md5($data['e'].$data['t']), 8, 16);
                if ($key_str!=$data['k']) {
                    $this->_404('找回密码失败,key错误!', 'user_getpass');
                }
                if (!$uid = M('Members')->where(array('email'=>$data['e']))->getfield('uid')) {
                    $this->_404('找回密码失败,帐号不存在!', 'user_getpass');
                }
                $token=substr(md5(mt_rand(100000, 999999)), 8, 16);
    ...
    public function oauth_reg()
    {
        ...
        if (false === $this->visitor->login($uid)) {
            $this->_404($this->visitor->getError());
        }
        if ($user_bind_info['type'] == 'weixin') {
            ...
            if (!$this->visitor->get('avatars') && $user_bind_info['keyavatar_big']) {
                $reg = M('Members')->where(array('uid'=>$uid))->setfield('avatars', $user_bind_info['keyavatar_big']);//临时头像转换
            }
        } else {
            $oauth = new \Common\qscmslib\oauth($user_bind_info['type']);
            $bind_user = $oauth->_checkBind($user_bind_info['type'], $user_bind_info['keyid']);
            if ($bind_user['uid'] && $bind_user['uid'] != $uid) {
                $this->_404('此帐号已经绑定过本站！');
            }
            $user_bind_info['uid'] = $uid;
            if (false === $oauth->bindUser($user_bind_info)) {
                $this->_404('帐号绑定失败，请重新操作！');
            }
            if (!$this->visitor->get('avatars') && $user_bind_info['temp_avatar']) {
    ...
    {
        if (!C('qscms_weixin_apiopen') || !C('qscms_weixin_mpname')) {
            $this->ajaxReturn(0, '未配置微信参数！');
        }
