    ...
    public function resume_add()
    {
        ...
        if (IS_POST && IS_AJAX) {
            if (C('qscms_sms_open')==1 && C('qscms_login_per_audit_mobile')==1 && !C('visitor.mobile_audit')) {
                ...
                if (false === $reg = M('Members')->where(array('uid'=>C('visitor.uid')))->save($setsqlarr)) {
    ...
    */
    public function resume_edit_intent()
    {
        ...
        if (IS_AJAX) {
            ...
            if ($rst['state']) {
                $this->ajaxReturn(1, '求职意向修改成功！', $data);
            }
            $this->ajaxReturn(0, $rst['error']);
        }
        $category = D('Category')->get_category_cache();
    ...
    */
    ...
    public function shield_company_add()
    {
        ...
        if (false === $data['id'] = $shield_mod->add()) {
            $this->ajaxReturn(0, '企业关键字添加失败，请重新添加！');
        }
        //写入会员日志
