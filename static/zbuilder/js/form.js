...
jQuery(document).ready(function () {
                    ...
                    .done(function (res) {
                        ...
                        if (res.code == '1') {
                            $thumbnail
                                .show()
                                .find('img')
                                .attr('src', res.thumb || res.src)
                                .attr('data-original', res.src);
    ...
    $('.js-editormd').each(function () {
        ...
        editormds[editormd_name] = editormd(editormd_name, {
            ...
            toolbarIcons: function () {
                return ['undo', 'redo', '|', 'bold', 'del', 'italic', 'quote', '|', 'h1', 'h2', 'h3', 'h4', 'h5', 'h6', '|', 'list-ul', 'list-ol', 'hr', '|', 'link', 'reference-link', 'image', 'code', 'preformatted-text', 'code-block', 'datetime', 'html-entities', 'pagebreak', '|', 'goto-line', 'watch', 'preview', 'fullscreen', 'clear', 'search', '|', 'help', 'info'];
            },
        });
    });
    ...
    $('.js-upload-image,.js-upload-images').each(function () {
        ...
        var $urlflag = $input_file.data('urlflag');
        ...
        uploader.on('fileQueued', function (file) {
            var $li = $(
                    '<div id="' +
                        file.id +
                        '" class="file-item js-gallery thumbnail">' +
                        '<img>' +
                        '<div class="info">' +
                        file.name +
                        '</div>' +
                        ...
                        for (var item in $targets) {
                            ...
                            if (dolphin._field_clear[trigger] !== undefined && dolphin._field_clear[trigger] === 1) {
                                ...
                                if ($form_item.find("[name='" + $targets[item] + "']").attr('type') == 'radio') {
                                    $form_item
                                        .find("[name='" + $targets[item] + "']:checked")
                                        .prop('checked', false)
                                        .trigger('change');
